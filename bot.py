...
function step2Next(){
  ...
  if(!dt.value) dt.value=now.toISOString().slice(0,16);
...
function saveTest(){
  ...
  // Saqlash tugmasini bloklash (ikki marta bosilmasin)
  ...
  // Vaqtni chiroyli ko'rsatish uchun
  function fmtDT(d){
  ...
  // Avval xabar ko'rsatish, keyin yopish
  tg.showPopup({
    title:'✅ Test muvaffaqiyatli saqlandi!',
    message:
      '🔑 Kod: '+meta.code+'\n'+
      '📚 Fan: '+meta.subject+'\n'+
      '📊 Jami: '+(meta.open_count+meta.closed_count)+' ta savol\n'+
      '🕐 Boshlanish: '+fmtDT(startDt)+'\n'+
      '🕑 Tugash: '+fmtDT(endDt),
    buttons:[{id:'ok',type:'ok',text:'Yopish'}]
  }, function(){
    // Popup yopilgandan keyin data yuborib oyna yopiladi
