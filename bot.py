...
// ── Vaqt yordamchilari ──
// datetime-local string → mahalliy Date obyekti (UTC EMAS!)
function localStrToDate(val){
  // "2024-11-25T14:30" → lokal vaqt sifatida parse qilinadi
  const [date,time]=val.split('T');
  const [y,m,d]=date.split('-').map(Number);
  const [h,min]=(time||'00:00').split(':').map(Number);
  return new Date(y,m-1,d,h,min); // new Date(y,m,d,h,min) DOIM lokal vaqt!
}
// Date → datetime-local input qiymati (lokal vaqt)
function dateToLocal(dt){
  const p=n=>String(n).padStart(2,'0');
  return dt.getFullYear()+'-'+p(dt.getMonth()+1)+'-'+p(dt.getDate())+
         'T'+p(dt.getHours())+':'+p(dt.getMinutes());
}
// Date → chiroyli ko'rsatish
function fmtShow(dt){
  const p=n=>String(n).padStart(2,'0');
...
function step2Next(){
  ...
  const now=new Date();
  const def=new Date(now.getTime()+10*60000); // 10 daqiqa keyin
  const dt=document.getElementById('startTime');
  if(!dt.value) dt.value=dateToLocal(def); // Lokal vaqt!
  dt.min=dateToLocal(now);
  goStep(3);
}
...
async function saveTest(){
  ...
  // Lokal vaqtdan to'g'ri Date yasash
  const startDt=localStrToDate(st);
  const endDt=new Date(startDt.getTime()+dur*60000);

  // Tugmani bloklash
  const btn=document.querySelector('.btn-grn');
  btn.disabled=true;
  btn.textContent='⏳ Saqlanmoqda...';

  const ownerTgId=tg.initDataUnsafe?.user?.id||0;
    ...
    if(result.success){
      ...
      document.querySelector('.container').innerHTML=`
        <div style="text-align:center;padding:40px 20px">
          ...
          <h2 style="color:#27ae60;margin-bottom:20px">Test Saqlandi!</h2>
          <div style="background:#eafaf1;border:2px solid #27ae60;border-radius:14px;padding:20px;text-align:left;margin-bottom:24px">
            <div style="margin-bottom:10px;font-size:15px">🔑 <b>Kod:</b> ${result.code}</div>
            <div style="margin-bottom:10px;font-size:15px">📚 <b>Fan:</b> ${result.subject}</div>
            <div style="margin-bottom:10px;font-size:15px">📊 <b>Jami:</b> ${result.total} ta savol</div>
            <div style="margin-bottom:10px;font-size:15px">🕐 <b>Boshlanish:</b> ${result.start}</div>
            <div style="font-size:15px">🕑 <b>Tugash:</b> ${result.end}</div>
          </div>
          <p style="color:#666;font-size:14px;margin-bottom:20px">
            Chatda ham tasdiqlash xabari yuborildi ✅
          </p>
          <button onclick="tg.close()" style="
            background:#27ae60;color:#fff;border:none;border-radius:10px;
            padding:14px 40px;font-size:16px;font-weight:700;cursor:pointer;width:100%
          ">✅ Yopish</button>
        </div>`;
    } else {
      btn.disabled=false;
...
def api_create_test():
    """WebApp dan fetch() orqali test yaratish"""
    ...
    owner_tg_id  = int(data.get("owner_tg_id", 0))
    ...
    # Telegram orqali egaga xabar yuborish
    ...
    try:
        req_lib.post(
            ...
            json={"chat_id": owner_tg_id, "text": msg_text, "parse_mode": "Markdown"},
            timeout=5
        )
